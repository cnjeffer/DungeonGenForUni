...
public class ObjectSpawner : MonoBehaviour
{
    ...
    [Header("Appearance")]
    public GameObject[] BrokenPillarPrefabs;
    ...
    public void Spawn(int count)
    {
        for (var i = 0; i < count; i++)
        {
            ...
            PlaceBrokenPillarAt(hit.point);
        }
    }

    //Given a position that we successfully touch the terrain at, 
    public void PlaceBrokenPillarAt(Vector3 position)
    {
        //Spawns a random prefab at the provided position - and gives it a random rotation and scale
        var newObj = Instantiate(BrokenPillarPrefabs[Random.Range(0, BrokenPillarPrefabs.Length)]);
